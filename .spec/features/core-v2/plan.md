...
| Rerank ordering + formatter output fields | core-v2/6 (done) | [tech.md](tech.md) § Search + scores (R11) realized: `CoreV2RerankConfig` (`[core.v2.rerank]`, disabled default) registered after `core.v2.search`; `retrieval.search` applies `SentenceTransformerRerank` per collection AFTER the cosine `score_threshold` filter (threshold is a cosine cutoff; rerank replaces scores with CE scores + keeps `top_n`), all imports function-local. For R11 the MCP formatter ADDS a unified `relevance` + a `score_kind` label ("cosine"/"l2_squared") per chunk in the mixed/v2-present path while preserving the raw `relevance_score`; the CLI reorders on the same unified measure. Both apply ONLY when a v2 collection is present, so v1-only output stays byte-identical (R6) |
