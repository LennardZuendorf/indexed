...
class ConfigService:
    ...
    def _filter_known_top_level(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if unknown:
            logging.getLogger(__name__).warning(
                "Ignoring unknown top-level config keys in indexed.toml: %s", ", ".join(unknown)
            )
        return {k: v for k, v in data.items() if k in allowed}

    def get(self, profile: Optional[str] = None, overrides: Optional[Dict[str, Any]] = None) -> IndexedSettings:
        ...
        if profile is None and overrides is None and ConfigService._settings_cache is not None:
        ...
        if profile:
            ...
            if 'profiles' in toml_data and profile in toml_data['profiles']:
                base_config = self._deep_merge(base_config, toml_data['profiles'][profile])
            if overrides:
                base_config = self._deep_merge(base_config, overrides)
            try:
                return IndexedSettings.model_validate(base_config)
            except Exception as exc:
                raise ValueError(f"Invalid configuration with profile '{profile}': {exc}") from exc
        ...
        if overrides:
            ...
            try:
                ...
                raise ValueError(f"Invalid configuration with overrides: {exc}") from exc
        else:
    ...
    def update(self, patch: Dict[str, Any], profile: Optional[str] = None) -> None:
        ...
        if profile:
            ...
            existing['profiles'][profile] = self._deep_merge(existing['profiles'][profile], patch)
        else:
...
def get_config(profile: Optional[str] = None, overrides: Optional[Dict[str, Any]] = None) -> IndexedSettings:
