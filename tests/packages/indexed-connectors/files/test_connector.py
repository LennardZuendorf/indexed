...
def test_filesystem_connector_from_config_minimal():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        mock_config = MagicMock()
        mock_settings = MagicMock()
        mock_settings.test = MagicMock(path=tmpdir)
        # Don't set optional attributes - they should use None/defaults
        mock_config.get.return_value = mock_settings
...
def test_filesystem_connector_from_config_missing_path():
    ...
    with pytest.raises(ValueError, match="FileSystem connector config at 'test' requires 'path'"):
