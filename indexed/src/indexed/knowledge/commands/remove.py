...
def remove(
    ...
    console.print()
    console.print(f"[{get_heading_style()}]Removing [{get_accent_style()}]{collection}[/{get_accent_style()}] Collection:[/{get_heading_style()}]")
    console.print()
    ...
    lines.append(create_info_row("Documents", str(target_collection.number_of_documents)))
    lines.append(create_info_row("Chunks", str(target_collection.number_of_chunks)))
    if target_collection.disk_size_bytes:
        lines.append(create_info_row("Size", format_size(target_collection.disk_size_bytes)))
    if target_collection.updated_time:
        lines.append(create_info_row("Updated", format_time(target_collection.updated_time)))
    ...
    if not force:
        console.print()
        console.print(f"[{get_heading_style()}]You are about to remove:[/{get_heading_style()}] {target_collection.number_of_documents} documents, {target_collection.number_of_chunks} chunks, {format_size(target_collection.disk_size_bytes)} size.")
        
        if not Confirm.ask(f"[{get_error_style()}]This action cannot be undone! Continue?[/{get_error_style()}]", default=False):
            console.print(f"[{get_dim_style()}]Cancelled[/{get_dim_style()}]")
            raise typer.Exit(0)
    
    # Execute removal
    try:
        console.print(f"[{get_dim_style()}]Removing collection '{collection}'...[/{get_dim_style()}]")
        index.remove(collection)
        console.print(f"[{get_default_style()}]✓ Collection '{collection}' removed[/{get_default_style()}]")
