...
from indexed.utils.components.theme import get_accent_style, get_label_style, get_default_style
...
def wrap_generator_with_progress_bar(
    ...
    if _cli_progress and _cli_console:
        ...
        task_id = _cli_progress.add_task(
            progress_bar_name,
            total=approx_total
        )
    ...
    def callback(update):
        ...
        if update.total and update.total > 0:
            ...
            progress.update(
                task_id,
                completed=completed,
                description=f"[bold blue]{update.message}"
            )
        else:
...
def create_standard_progress() -> Progress:
    ...
    return Progress(
        SpinnerColumn(),
        TextColumn("[bold #2581C4]{task.description}"),  # Blue accent for operation text
        BarColumn(
...
def create_operation_progress(operation_desc: str, total: int = 100):
    ...
    with create_standard_progress() as progress:
        ...
        try:
            ...
            styled_name = f"[{get_label_style()}]{collection_name}[/{get_label_style()}]"
            progress.update(task_id, description=f"{styled_name}[{get_default_style()}]: Starting...[/{get_default_style()}]")
            ...
            if elapsed < 0.2:
                ...
                # Style: collection name in label style, rest in default style
                styled_name = f"[{get_label_style()}]{collection_name}[/{get_label_style()}]"
                progress.update(task_id, description=f"✓ {styled_name}[{get_default_style()}]: Complete[/{get_default_style()}]")
                time.sleep(0.1)  # Brief pause to show completion
            else:
                # Operation took long enough, show full progress completion
                # Style: collection name in label style, rest in default style
                styled_name = f"[{get_label_style()}]{collection_name}[/{get_label_style()}]"
                progress.update(task_id, completed=total, description=f"✓ {styled_name}[{get_default_style()}]: Complete[/{get_default_style()}]")
