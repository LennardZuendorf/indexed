...
def set_config(
    ...
    if key not in key_mapping:
        ...
        typer.echo(
            "Available keys: embedding_model, chunk_size, storage_path", err=True
        )
        raise typer.Exit(1)
    ...
    if attr_name == "chunk_size":
        try:
            ...
            typer.echo(
                f"❌ Error: chunk_size must be an integer, got '{value}'", err=True
            )
            raise typer.Exit(1)
...
def validate():
    ...
    try:
        ...
        if config.chunk_size <= 0:
            typer.echo("❌ Configuration validation failed:", err=True)
            typer.echo(
                f"  • chunk_size must be positive, got {config.chunk_size}", err=True
            )
            raise typer.Exit(1)
