...
def _execute_fastmcp(cmd: List[str]) -> None:
    ...
    try:
        ...
        typer.echo("❌ Error: FastMCP CLI not found. Please install it with: pip install fastmcp", err=True)
        raise typer.Exit(1)
...
def _get_common_fastmcp_params():
    ...
    return {
        'python': typer.Option(None, "--python", help="Python version to use (e.g., 3.11)"),
        'with_packages': typer.Option([], "--with", help="Additional packages to install (can be used multiple times)"),
        'with_requirements': typer.Option(None, "--with-requirements", help="Requirements file to install dependencies from"),
        'with_editable': typer.Option(None, "--with-editable", "-e", help="Directory containing pyproject.toml to install in editable mode"),
        'project': typer.Option(None, "--project", help="Run the command within the given project directory"),
        'skip_env': typer.Option(False, "--skip-env", help="Skip environment setup with uv (use when already in a uv environment)"),
        'path': typer.Option(None, "--path", help="Path to bind to when using http transport"),
        'no_banner': typer.Option(False, "--no-banner", help="Disable the startup banner display"),
    }
...
def run(
    ...
    python: Optional[str] = typer.Option(None, "--python", help="Python version to use (e.g., 3.11)"),
    with_packages: Optional[List[str]] = typer.Option([], "--with", help="Additional packages to install (can be used multiple times)"),
    with_requirements: Optional[str] = typer.Option(None, "--with-requirements", help="Requirements file to install dependencies from"),
    with_editable: Optional[str] = typer.Option(None, "--with-editable", "-e", help="Directory containing pyproject.toml to install in editable mode"),
    project: Optional[str] = typer.Option(None, "--project", help="Run the command within the given project directory"),
    skip_env: bool = typer.Option(False, "--skip-env", help="Skip environment setup with uv (use when already in a uv environment)"),
    path: Optional[str] = typer.Option(None, "--path", help="Path to bind to when using http transport"),
    no_banner: bool = typer.Option(False, "--no-banner", help="Disable the startup banner display"),
):
...
def dev(
    ...
    path: Optional[str] = typer.Option(None, "--path", help="Path to bind to when using http transport"),
    no_banner: bool = typer.Option(False, "--no-banner", help="Disable the startup banner display"),
    inspector_version: Optional[str] = typer.Option(None, "--inspector-version", help="Version of the MCP Inspector to use"),
    ui_port: Optional[int] = typer.Option(None, "--ui-port", help="Port for the MCP Inspector UI"),
    server_port: Optional[int] = typer.Option(None, "--server-port", help="Port for the MCP Inspector Proxy server"),
):
...
@app.command("inspect")
def inspect(
    format: Optional[str] = typer.Option(None, "--format", help="Output format: text, fastmcp, or mcp"),
