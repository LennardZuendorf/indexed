...
ap.add_argument(
    "-jql", "--jql", required=True, help="Jira query (JQL) to get tickets for indexing"
)
...
if is_cloud:
    ...
    jira_document_reader = JiraCloudDocumentReader(
        base_url=args["url"], query=args["jql"], email=email, api_token=api_token
    )
    ...
    jira_document_reader = JiraDocumentReader(
        base_url=args["url"],
        query=args["jql"],
        token=token,
        login=login,
...
jira_collection_creator = create_collection_creator(
