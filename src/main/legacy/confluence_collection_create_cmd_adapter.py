...
ap.add_argument(
    ...
    "--cql",
    required=True,
    help="Confluence query (CQL) to get pages for indexing",
)
...
if is_cloud:
    ...
    confluence_document_reader = ConfluenceCloudDocumentReader(
        base_url=args["url"],
        query=args["cql"],
        email=email,
        api_token=api_token,
    ...
    confluence_document_reader = ConfluenceDocumentReader(
        base_url=args["url"],
        query=args["cql"],
        token=token,
        login=login,
...
confluence_collection_creator = create_collection_creator(
