...
class PathsSettings(BaseModel):
    ...
    collections_dir: str = Field(default="./data/collections", description="Directory for collections storage")
    caches_dir: str = Field(default="./data/caches", description="Directory for document caches")
...
class SearchSettings(BaseModel):
    ...
    max_chunks: Optional[int] = Field(default=30, ge=1, description="Maximum chunks to return")
    include_full_text: bool = Field(default=False, description="Include full document text in results")
    include_all_chunks: bool = Field(default=False, description="Include all document chunks")
    include_matched_chunks: bool = Field(default=False, description="Include only matched chunks")
...
class IndexSettings(BaseModel):
    ...
    embedding_batch_size: int = Field(default=64, ge=1, description="Batch size for embedding generation")
...
class FilesSourceSettings(BaseModel):
    ...
    base_path: Optional[str] = Field(default=None, description="Base directory path for file indexing")
...
class JiraCloudSourceSettings(BaseModel):
    ...
    email: Optional[str] = Field(default=None, description="Jira Cloud email for authentication")
    api_token_env: str = Field(default="JIRA_API_TOKEN", description="Environment variable containing API token")
    jql: str = Field(default="project = CURRENT", description="JQL query for issue selection")
    max_results: int = Field(default=100, ge=1, description="Maximum results per API call")
...
class ConfluenceCloudSourceSettings(BaseModel):
    ...
    base_url: Optional[str] = Field(default=None, description="Confluence Cloud base URL")
    email: Optional[str] = Field(default=None, description="Confluence Cloud email for authentication")
    api_token_env: str = Field(default="CONFLUENCE_API_TOKEN", description="Environment variable containing API token")
    cql: str = Field(default="space = DEV", description="CQL query for content selection")
    include_comments: bool = Field(default=False, description="Include page comments in indexing")
...
class SourcesSettings(BaseModel):
    ...
    confluence_cloud: ConfluenceCloudSourceSettings = Field(default_factory=ConfluenceCloudSourceSettings)
...
class PerformanceSettings(BaseModel):
    ...
    cache_max_entries: int = Field(default=32, ge=1, description="Maximum cache entries")
    log_sqlite_queries: bool = Field(default=False, description="Log SQLite queries for debugging")
