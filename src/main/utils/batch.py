...
def read_items_in_batches(
    ...
    while are_there_more_items_to_read:
        current_batch_size = (
            batch_size if number_of_items_to_read_one_by_one == 0 else 1
        )
        try:
            if cursor_parser is not None:
                read_result = read_batch_func(
                    start_at, current_batch_size, cursor=cursor
                )
            else:
            ...
            if current_batch_size == 1:
                ...
                start_at += 1
                are_there_more_items_to_read = (
                    start_at < total if total is not None else True
                )
